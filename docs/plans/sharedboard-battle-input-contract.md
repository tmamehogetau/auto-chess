...
- 根拠:
  - `src/server/rooms/shared-board-room.ts` (`emitPlacementChange` 呼び出し)
  - `src/server/rooms/shared-board-room.ts` (変更イベント作成)
  - `src/server/rooms/shared-board-room.ts` (`gamePlayerId` を優先した論理 owner 解決)
...
実装済み:
- `battle_trace`: 採用された `player placements` を戦闘単位で出力
- `battle_result_trace`: 勝敗・生存ユニット数・適用ダメージを同一 `battleId` で相関可能
...
以下がCIで安定パスすること:
- 正常系: Prep中の共有編集が戦闘入力へ反映され、`battle_trace` でも採用配置を確認できる
