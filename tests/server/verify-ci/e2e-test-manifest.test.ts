...
describe("e2e test manifest", () => {
  test("covers every tests/e2e suite exactly once", async () => {
    ...
    expect(new Set([...shardA, ...shardB])).toEqual(new Set(allTests));
  ...
  test("helper CLI lists both shards as tests/e2e files", async () => {
    const scriptPath = path.resolve(process.cwd(), "scripts/e2e-test-manifest.mjs");
    const [{ stdout: shardAStdout }, { stdout: shardBStdout }] = await Promise.all([
    ...
    expect(shardA.length + shardB.length).toBe(allTests.length);
  });
});
